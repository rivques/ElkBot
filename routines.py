...
class catch():
    #Catches the ball on top of the car to prepare for dribbling
    def __init__(self, hits_ground_location):
        self.hits_ground_location = hits_ground_location
    def run(self, agent):
        relative_target = self.hits_ground_location.flatten() - agent.me.location
        local_target = agent.me.local(relative_target)
        defaultPD(agent, local_target)
        defaultThrottle(agent, relative_target.magnitude())
        if abs((agent.ball.location-agent.me.location).magnitude()) < 200:
            agent.pop()
...
class goto():
    ...
    def __init__(self, target, vector=None, boost=False, direction = 1):
        self.target = target
        self.vector = vector
    ...
    def run(self,agent):
        ...
        agent.controller.boost = self.boost
        agent.controller.handbrake = True if abs(angles[1]) > 2.3 else agent.controller.handbrake
